...
(async () => {
  ...
  await test('bulk import API (3 students) succeeds with pre-hashed passwords', async () => {
    const students = [
      { 'الاسم': 'بلك تيست ١', 'المرحلة': 'الصف الأول الثانوي عام', 'الجنس': 'ذكر' },
      { 'الاسم': 'بلك تيست ٢', 'المرحلة': 'الصف الثاني الثانوي عام', 'الجنس': 'أنثى' },
  ...
  await test('bulk import: empty-name row skipped, valid row created', async () => {
    const res = await request('POST', '/api/students/bulk', {
      students: [
        { 'الاسم': '',         'المرحلة': 'الصف الأول الثانوي عام' },
        { 'الاسم': 'طالب صالح', 'المرحلة': 'الصف الأول الثانوي عام' },
      ]
    }, TEST_TOKEN);
