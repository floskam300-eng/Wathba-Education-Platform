...
router.get('/import-model', requireRole('teacher', 'assistant'), async (req, res) => {
  ...
  try {
    ...
    res.json({ model: result.rows[0] || null });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error' });
  }
...
router.post('/import-model', requireRole('teacher', 'assistant'), async (req, res) => {
  ...
  if (!Array.isArray(headers) || !headers.length) {
    return res.status(400).json({ error: 'يجب توفير أعمدة الملف' });
  }
  if (!mappings || typeof mappings !== 'object') {
    return res.status(400).json({ error: 'يجب توفير خريطة الأعمدة' });
  }
  if (!mappings.name) {
    return res.status(400).json({ error: 'يجب ربط عمود اسم الطالب على الأقل' });
  }
  const ALLOWED_FIELDS = ['name','phone','parent_phone','username','password','gender','academic_stage'];
  const cleanMappings = {};
  for (const f of ALLOWED_FIELDS) {
    if (mappings[f] && typeof mappings[f] === 'string') cleanMappings[f] = mappings[f].slice(0, 200);
  }
  try {
    await pool.query(
      `INSERT INTO teacher_import_models (teacher_id, headers, sample_row, mappings)
       ...
       ON CONFLICT (teacher_id) DO UPDATE SET
         ...
         updated_at = NOW()`,
      [teacherId, JSON.stringify(headers.map(h => String(h).slice(0, 200))), JSON.stringify(sample_row || {}), JSON.stringify(cleanMappings)]
    );
