...
ALTER TABLE teachers ADD COLUMN IF NOT EXISTS support_form_url VARCHAR(500);
...
-- Subject definitions per teacher
...
-- Daily attendance record per student per subject per date
...
-- Ensure all session, exam, recitation, live, and activity timestamp columns use TIMESTAMPTZ for global timezone alignment
DO $$
BEGIN
