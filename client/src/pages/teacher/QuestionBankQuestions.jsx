        ...
        <div className="px-4 lg:px-6 py-3 flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <p className="text-[10px] sm:text-xs text-gray-400 font-medium">إدارة أسئلة البنك</p>
              <h1 className="font-black text-navy-700 text-xs sm:text-sm truncate">{bank?.name || '...'}</h1>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-3 order-2 lg:order-1">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-black text-navy-700 text-base">
                الأسئلة
                <span className="mr-2 text-sm font-semibold text-gray-500">({bankQuestions.length})</span>
              </h2>
            </div>
            ...
            {isLoading ? (
              ...
              bankQuestions.map((q, idx) => {
                ...
                return (
                  <div key={q.id} className={`bg-white rounded-xl p-4 shadow-sm border transition-all ${editQ?.id === q.id ? 'border-purple-400 ring-2 ring-purple-100' : 'border-gray-100 hover:border-gray-200'}`}>
                          ...
                          <div className="grid grid-cols-2 gap-1.5">
                            {['a','b','c','d'].map((opt, oidx) => q[`option_${opt}`] && (
                              <div key={opt} className={`px-2 py-1 rounded-lg text-xs font-medium border ${q.correct_answer_letter?.toUpperCase() === opt.toUpperCase() ? 'border-green-400 bg-green-50 text-green-800 font-bold' : 'border-gray-200 text-gray-600'}`}>
                                <span className="font-black ml-1">{q.option_labels?.[oidx] || opt.toUpperCase()}.</span>{q[`option_${opt}`]}
                              </div>
                            ))}
                          </div>
                        )}
            ...
            <div className="bg-white rounded-2xl border-2 border-dashed border-purple-300 p-5 shadow-sm">
              ...
              <form onSubmit={handleQSubmit} className="space-y-3">
                      ...
                      return (
                        <div key={opt} className="flex items-center gap-2">
                          ...
                          <input value={opt === 'A' ? qForm.option_a : opt === 'B' ? qForm.option_b : opt === 'C' ? qForm.option_c : qForm.option_d}
                            ...
                            className="input-field text-sm flex-1" placeholder={`الخيار ${displayLabel}${opt === 'A' || opt === 'B' ? ' *' : ' (اختياري)'}`} />
                        </div>
                      );
                    })}
                ...
                <div className="flex gap-2 pt-1 border-t border-gray-100">
                  {editQ && (
                    <button type="button" onClick={resetQForm} className="btn-secondary px-4 py-2 text-sm flex-shrink-0">إلغاء</button>
                  )}
                  <button type="submit" disabled={addQMut.isPending || updateQMut.isPending} className="btn-primary flex-1 flex items-center justify-center gap-2">
                    {editQ ? 'حفظ التعديل' : <><Plus className="w-4 h-4" /> إضافة سؤال</>}
                  </button>
                </div>
              </form>
            </div>
