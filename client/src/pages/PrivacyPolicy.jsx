...
const Section = ({ icon: Icon, title, children, accent = 'orange' }) => (
  <div className="mb-10">
    <div className="flex items-center gap-3 mb-4">
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${accent === 'purple' ? 'bg-purple-100 border border-purple-200' : 'bg-orange-100 border border-orange-200'}`}>
        <Icon className={`w-4.5 h-4.5 ${accent === 'purple' ? 'text-purple-600' : 'text-orange-500'}`} style={{ width: 18, height: 18 }} />
      </div>
      <h2 className={`text-lg font-black ${accent === 'purple' ? 'text-purple-700' : 'text-orange-600'}`}>{title}</h2>
    </div>
    <div className="text-[#0B3C5D]/65 text-sm leading-relaxed space-y-3 pr-12">
      {children}
    </div>
  </div>
);
...
export default function PrivacyPolicy() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#F8FAFC] text-[#0B3C5D]" style={{ fontFamily: "'Cairo', sans-serif" }}>
      <style>{`
        @keyframes pp-orb { from { transform: translate(0,0) scale(1); } to { transform: translate(15px,-20px) scale(1.06); } }
        .pp-dot-grid {
      ...
      {/* Navbar */}
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-[#0B3C5D]/10">
        <div className="max-w-4xl mx-auto px-5 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3">
            <img src={wathbaLogo} alt="وثبة" className="h-9 w-auto rounded-xl shadow-sm" />
          </Link>
          <Link to="/"
            className="flex items-center gap-2 text-[#0B3C5D]/60 hover:text-orange-500 text-sm font-semibold transition-colors">
      ...
      {/* Hero */}
      <div className="relative overflow-hidden pp-dot-grid py-16 mb-2 bg-gradient-to-br from-white via-[#F4F6F9] to-[#EBF0F7]">
        <div style={{ position: 'absolute', width: 500, height: 500, top: -200, right: -100, borderRadius: '50%', background: 'radial-gradient(circle,rgba(249,115,22,0.08),transparent 70%)', filter: 'blur(60px)', animation: 'pp-orb 12s ease-in-out infinite alternate', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', width: 400, height: 400, bottom: -150, left: -80, borderRadius: '50%', background: 'radial-gradient(circle,rgba(124,58,237,0.07),transparent 70%)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        <div className="relative z-10 max-w-4xl mx-auto px-5 text-center">
          <div className="inline-flex items-center gap-2 bg-orange-100 border border-orange-200 text-orange-600 text-xs font-bold px-4 py-1.5 rounded-full mb-5 tracking-widest uppercase">
            <Shield className="w-3.5 h-3.5" />
            سياسة الخصوصية
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-[#0B3C5D] mb-4 leading-tight">
            نحن نحمي<br />
          ...
          <p className="text-[#0B3C5D]/55 text-base max-w-xl mx-auto leading-relaxed">
            هذه الصفحة توضح بشفافية كاملة ما نجمعه من بيانات، وكيف نستخدمها، وكيف نحميها على منصة وثبة التعليمية.
          </p>
          <p className="text-[#0B3C5D]/30 text-xs mt-4">آخر تحديث: مايو ٢٠٢٦</p>
        </div>
      </div>

      {/* Content */}
      <div className="max-w-4xl mx-auto px-5 py-12">
        <div className="bg-white border border-slate-200 rounded-2xl p-8 md:p-12" style={{ boxShadow: '0 4px 24px rgba(11,60,93,0.08)' }}>

          {/* Intro */}
          <div className="bg-orange-50 border border-orange-200 rounded-xl p-5 mb-10">
            <p className="text-[#0B3C5D]/70 text-sm leading-relaxed">
              <span className="text-orange-600 font-bold">منصة وثبة</span> هي منصة تعليمية متكاملة مصممة خصيصاً لمدرسي السناتر في مصر. نحن نلتزم بحماية خصوصية جميع مستخدمي المنصة — المعلمين والمساعدين والطلاب وأولياء الأمور — ولا نتعامل مع بياناتك الشخصية إلا وفق ما هو موضح في هذه السياسة.
            </p>
          </div>

          <Section icon={Database} title="البيانات التي نجمعها">
            <p className="font-semibold text-[#0B3C5D]/80 mb-2">أولاً: بيانات المعلمين (أصحاب المنصات)</p>
            <BulletList items={[
            ...
            <p className="font-semibold text-[#0B3C5D]/80 mb-2 mt-4">ثانياً: بيانات الطلاب</p>
            <BulletList items={[
            ...
            <p className="font-semibold text-[#0B3C5D]/80 mb-2 mt-4">ثالثاً: بيانات المدفوعات والتسجيل</p>
            <BulletList items={[
            ...
            <p className="font-semibold text-[#0B3C5D]/80 mb-2 mt-4">رابعاً: بيانات الأجهزة (نظام الحماية)</p>
            <BulletList items={[
            ...
            <p className="font-semibold text-[#0B3C5D]/80 mb-2 mt-4">خامساً: بيانات الاستخدام التلقائية</p>
            <BulletList items={[
          ...
          <Section icon={Users} title="من يرى بياناتك؟">
            ...
            <p className="font-semibold text-[#0B3C5D]/80 mt-3 mb-2">الأطراف التي قد ترى بياناتك:</p>
            <BulletList items={[
          ...
          {/* Contact */}
          <div className="mt-10 bg-gradient-to-br from-orange-50 to-purple-50 border border-orange-200 rounded-2xl p-6">
          ...
          {/* Note */}
          <p className="text-[#0B3C5D]/30 text-xs mt-8 text-center leading-relaxed">
        ...
        <div className="text-center mt-8 flex items-center justify-center gap-6">
          <Link to="/" className="text-[#0B3C5D]/50 hover:text-orange-500 text-sm font-semibold transition-colors flex items-center gap-2">
            <ArrowRight className="w-4 h-4" />
            العودة للصفحة الرئيسية
          </Link>
          <Link to="/terms" className="text-[#0B3C5D]/50 hover:text-orange-500 text-sm font-semibold transition-colors">
            الشروط والأحكام
          </Link>
        </div>
      </div>
      ...
      {/* Footer */}
      <footer className="border-t border-[#0B3C5D]/10 py-6 mt-8 bg-white">
        <div className="max-w-4xl mx-auto px-5 text-center">
          <p className="text-[#0B3C5D]/30 text-xs">© {new Date().getFullYear()} منصة وثبة — جميع الحقوق محفوظة</p>
        </div>
      </footer>
    </div>
  );
