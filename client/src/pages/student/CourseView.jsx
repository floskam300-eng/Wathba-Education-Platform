...
function VideoPlayer({ video, onProgressUpdate, studentName, studentCode, initialPosition = 0 }) {
  ...
  // Drive videos AND uploaded videos both fall through here — the server
  // returns file_path_or_url = '/uploads/drive/<videoId>' for Drive videos
  // and '/uploads/videos/<file>' for uploads. Both go through the same
  // <video> element with media-token auth and full anti-cheat support.
    ...
    return (
      ...
      >
        ...
        <div className="px-3 pb-3 border-t border-gray-200 dark:border-white/10 pt-3 flex-shrink-0">
          <button
            onClick={() => navigate('/student/recitations', { state: { backTo: `/student/courses/${courseId}` } })}
