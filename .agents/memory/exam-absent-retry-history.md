...
## 7 more bugs fixed (second-pass audit, N1-N7)
| # | File | Issue | Fix |
|---|------|-------|-----|
| N1 | exams.js `/student/available` | Absent record leaked into `already_taken` → student saw retry UI | Added `AND er.is_absent = false` to the LEFT JOIN ON clause |
| N2 | exams.js `/retry-request` | Absent student could POST retry request (is_latest=true matches taken check) | Added `if (taken.rows[0].is_absent) return 400` |
| N3 | archive.js `/student/:id/summary` | `total_exams`, `failed_exams`, `avg_score` included absent records (score=0 inflated failed) | `AND er.is_absent=false` in all FILTER clauses; new `absent_exams` field |
| N4 | scheduler.js | `runEndedExamCheck` duplicated full absent-marking logic — DRY violation | Replaced 40-line duplicate with `await markAbsentStudents(_pool, exam.id, exam.teacher_id)` |
| N5 | student/Exams.jsx | Absent record showed retry UI — already covered by N1 server fix | N1 makes `already_taken=null` for absent students |
| N6 | StudentArchiveModal.jsx | PDF export: absent row printed as "راسب" (score=0 < pass_score) | `isAbsent ? 'غائب'` check in `examRows` mapping |
| N7 | StudentArchiveModal.jsx | Summary stat panel: no absent count; "راسب" inflated | Conditional `<StatPill label="غائب">` when `exE?.absent_exams > 0` |

## 9 bugs fixed (first audit pass)
| # | File | Issue | Fix |
|---|------|-------|-----|
...
- Test file: `server/tests/absent_retry_test.js` — 93 tests / 11 sections; all pass
- Section 11 (N1-N7): covers `already_taken` JOIN fix, absent retry-request guard, summary field exclusion, and scheduler DRY
